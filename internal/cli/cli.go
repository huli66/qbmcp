...
配置: %LOCALAPPDATA%\qbmcp\config.json
日志: %LOCALAPPDATA%\qbmcp\logs\qbmcp.log
默认 WebSocket: ws://127.0.0.1:32300/ws
...
func Run(args []string) error {
	...
	if command == "_run" || command == "_watch" || command == "_autostart" {
		...
		if command == "_run" {
			return host.RunBackground()
		}
		return host.RunWatch(command == "_autostart")
	}
	...
	return host.Manage(command, port, asJSON)
}
